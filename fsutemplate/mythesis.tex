...
Many consumer personal computers are equipped with powerful GPUs which can be
harnassed for general purpose computation.

This thesis presents a particle system library is accelerated by the GPU using
the OpenCL programming language.  The library integrated into the Blender Game
...
The primary system implemented in this research is a fluid simulator using the
Smoothed Particle Hydrodynamics technique for simulating incompressible fluids
such as water.
...
%\toi{double check bib}
%\toi{columnwidth for all images}
%\toi{short captions for figures}
%\toi{remove equation numbers from ones not mentioned}
